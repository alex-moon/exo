...
export class Terrain extends Ground {
  ...
  public constructor() {
    ...
    for (let r = 1; r <= Terrain.RINGS; r++) {
      ...
      for (let s = 0; s < Terrain.SPOKES; s++) {
        ...
        // 2. Concentric rings
        const nextSpokeIndex =
          1 + (r - 1) * Terrain.SPOKES + ((s + 1) % Terrain.SPOKES);
  ...
  public update(): void {
    ...
    if (!coordsChanged && now - this.lastUpdate < 500) return;
      ...
      try {
        ...
        const getLngLat = (x: number, y: number) => {
          ...
          const vLng =
            lng +
            metersX /
              ((GeoUtils.EQUATOR_CIRCUMFERENCE *
        ...
        for (let i = 0; i < positions.count; i++) {
          ...
          fetchPromises.push(
            provider.getAlt(vLng, vLat).then((alt) => {
              alts[i] = alt;
            })
          );
        }
