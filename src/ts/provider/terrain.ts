...
export class TerrainProvider {
    ...
    public async getAlt(lng: number, lat: number): Promise<number> {
        ...
        const percentY = (GeoUtils.lat2y(lat) - GeoUtils.lat2y(nwLat)) / (GeoUtils.lat2y(seLat) - GeoUtils.lat2y(nwLat));

        return this.sampleInterpolated(tileData, percentX, percentY);
    }

    private sampleInterpolated(data: Float32Array, percentX: number, percentY: number): number {
        const res = 256;
    ...
    private async getTileData(x: number, y: number, z: number): Promise<Float32Array> {
        const key = `${z}/${x}/${y}`;
    ...
    private async fetchTile(x: number, y: number, z: number): Promise<Float32Array> {
